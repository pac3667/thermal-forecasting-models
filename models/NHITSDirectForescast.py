...
from utils import optuna_darts_nhits_search


def train_nhits_direct_multistep(data_path, checkpoint_dir, n_out, train_size):
    """
    Полный Direct-пайплайн многошагового прогнозирования ТЭЦ на базе архитектуры NHITS.
    """
    step_checkpoint_dir = os.path.join(checkpoint_dir, 'multistep_nhits')
    os.makedirs(step_checkpoint_dir, exist_ok=True)

    model_checkpoint_path = os.path.join(step_checkpoint_dir, 'Qpred_nhits_direct_model.pt')
    params_json_path = os.path.join(step_checkpoint_dir, 'nhits_best_params.json')
    metrics_json_path = os.path.join(step_checkpoint_dir, 'nhits_metrics.json')

    # 1. Загрузка данных и первичная генерация признаков
    ...
    if os.path.exists(model_checkpoint_path) and os.path.exists(params_json_path) and os.path.exists(metrics_json_path):
        print("--- [Darts NHITS] Найден сохраненный пайплайн. Загрузка... ---")
        ...
        print("\n" + "=" * 60)
        print("=== ИЗВЛЕЧЕННЫЕ РЕЗУЛЬТАТЫ DARTS NHITS DIRECT ===")
        print("=" * 60)
    ...
    # === ОПТИМИЗАЦИЯ OPTUNA ===
    print("\nЗапуск Optuna для подбора гиперпараметров NHITS...")
    optuna.logging.set_verbosity(optuna.logging.WARNING)
    study = optuna.create_study(direction='minimize')
    study.optimize(lambda trial: optuna_darts_nhits_search(trial, base_data, train_size, n_out), n_trials=100)
    ...
    print(
        f"[УСПЕХ] Параметры подобраны. Окна: T — {best_t_smooth} дн, Q — {best_q_smooth} дн. Input Chunk: {input_chunk}")

    # 2. ПОДГОТОВКА ПРИЗНАКОВ С УЧЕТОМ ОПТИМАЛЬНОГО СГЛАЖИВАНИЯ
    data = base_data.copy()
    ...
    # Сборка единого набора ковариат
    ...
    early_stopper = EarlyStopping(monitor="train_loss", patience=10, min_delta=0.05, mode="min")

    # 3. ИНИЦИАЛИЗАЦИЯ МОДЕЛИ NHiTS
    ...
    print("Запуск финального обучения Direct-модели NHiTS...")
    model_nhits.fit(
        series=train_target,
        past_covariates=past_covariates
    )

    model_nhits.save(model_checkpoint_path)
    print(f"[SAVE SUCCESS] Веса NHiTS сохранены в: {model_checkpoint_path}")

    # 4. СКОЛЬЗЯЩИЙ ИНФЕРЕНС НА ТЕСТЕ
    ...
    for idx in range(train_size, max_test_idx):
        ...
        # Выполняем предсказание без future_covariates
        pred = model_nhits.predict(
            n=n_out,
    ...
    # 5. РАСЧЕТ И СОХРАНЕНИЕ МЕТРИК
    print("\n" + "=" * 50)
    print("=== РЕЗУЛЬТАТЫ ГОРИЗОНТА ДЛЯ DARTS NHITS DIRECT ===")
    print("=" * 50)
